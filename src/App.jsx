...
import {
  ...
  WalletCards, MonitorSmartphone, Store, MapPinned, Home
} from "lucide-react";
...
function StepButton({ active, icon: Icon, title, subtitle, onClick }) {
  return <button onClick={onClick} className={`flex min-h-[82px] items-center gap-3 rounded-3xl border p-4 text-left transition ${active ? "border-[#FFC72C] bg-[#FFF7D8] shadow-sm" : "border-[#E8D8B8] bg-white hover:bg-[#FFF7D8]"}`}>
    <div className={`rounded-2xl p-3 ${active ? "bg-[#FFC72C] text-slate-950" : "bg-[#F5F7FA] text-[#003E74]"}`}><Icon className="h-5 w-5" /></div>
    <div><p className="font-black text-slate-950">{title}</p><p className="mt-0.5 text-xs text-slate-500">{subtitle}</p></div>
  </button>;
}

function ModeSwitch({ mainMode, setMainMode, setOnlineStep, setNfcStep }) {
  return <div className="sticky top-[73px] z-10 rounded-[2rem] border border-[#E8D8B8] bg-white/95 p-3 shadow-sm backdrop-blur">
    <p className="mb-2 px-2 text-xs font-black uppercase tracking-wide text-slate-400">Choose service mode</p>
    <div className="grid grid-cols-2 gap-2">
      <button onClick={() => { setMainMode("online"); setOnlineStep("shops"); }} className={`rounded-2xl px-4 py-3 text-sm font-black transition ${mainMode === "online" ? "bg-[#FFC72C] text-slate-950" : "bg-[#F5F7FA] text-slate-600"}`}><MonitorSmartphone className="mx-auto mb-1 h-5 w-5" /> Online ordering</button>
      <button onClick={() => { setMainMode("nfc"); setNfcStep("scan"); }} className={`rounded-2xl px-4 py-3 text-sm font-black transition ${mainMode === "nfc" ? "bg-[#FFC72C] text-slate-950" : "bg-[#F5F7FA] text-slate-600"}`}><Nfc className="mx-auto mb-1 h-5 w-5" /> NFC tray</button>
    </div>
  </div>;
}
  ...
  return <div className="rounded-3xl bg-white p-4">
    ...
    <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${area.cols}, minmax(0, 1fr))` }}>
      {Array.from({ length: total }).map((_, index) => {
        ...
        return <div key={seatNo} className={`flex items-center justify-center rounded-lg border text-[10px] font-black transition ${large ? "h-10" : "h-7"} ${occupied ? "border-red-200 bg-red-500 text-white" : "border-[#E8D8B8] bg-white text-slate-300 hover:border-[#003E74] hover:text-[#003E74]"}`}>{occupied ? "●" : seatNo}</div>;
      })}
    </div>
...
export default function ImperialFoodWebpage() {
  ...
  const [onlineStep, setOnlineStep] = useState("shops");
  const [nfcStep, setNfcStep] = useState("scan");
  ...
  useEffect(() => {
    ...
    if (params.get("nfc") === "true") {
      ...
      const timer = setTimeout(() => setShowNfcConnect(false), 3000);
  ...
  const onlineSteps = [
    ["shops", Store, "1. Shops", "choose restaurant"],
    ["menu", Utensils, "2. Menu", "order or rate"],
    ["cart", ShoppingBag, "3. Cart", `${itemCount} items`],
    ["seats", MapPinned, "Seats", "optional"],
  ];
  const nfcSteps = [
    ["scan", ScanLine, "1. Scan", "detect dishes"],
    ["tray", ShoppingBag, "2. Tray", `${nfcItems.length} items`],
    ["seats", MapPinned, "Seats", "optional"],
  ];

  return <div className="min-h-screen bg-white pb-8 text-slate-950">
    <header className="sticky top-0 z-20 border-b border-[#E8D8B8] bg-white/95 px-5 py-4 backdrop-blur md:px-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        <div className="flex items-baseline gap-1"><h1 className="tracking-[0.28em] text-2xl font-black text-[#003E74]">IMPERIAL</h1><span className="text-xs font-black text-[#003E74]">Food</span></div>
        <button className="rounded-full border border-[#E8D8B8] bg-white p-2"><Menu className="h-5 w-5" /></button>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-7xl p-4 md:p-8">
      <div className="grid gap-5 lg:grid-cols-[290px_1fr]">
        <aside className="space-y-4 lg:sticky lg:top-24 lg:h-fit">
          <ModeSwitch mainMode={mainMode} setMainMode={setMainMode} setOnlineStep={setOnlineStep} setNfcStep={setNfcStep} />

          <div className="rounded-[2rem] border border-[#E8D8B8] bg-white p-3 shadow-sm">
            <p className="mb-2 px-2 text-xs font-black uppercase tracking-wide text-slate-400">Current flow</p>
            <div className="grid gap-2">
              {(mainMode === "online" ? onlineSteps : nfcSteps).map(([id, Icon, title, subtitle]) => (
                <StepButton key={id} active={(mainMode === "online" ? onlineStep : nfcStep) === id} icon={Icon} title={title} subtitle={subtitle} onClick={() => mainMode === "online" ? setOnlineStep(id) : setNfcStep(id)} />
              ))}
            </div>
          </div>
        </aside>

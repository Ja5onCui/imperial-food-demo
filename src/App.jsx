  ...
  return <div className="min-h-screen bg-white text-slate-950">
    <header className="sticky top-0 z-30 border-b border-[#E8D8B8] bg-white/95 px-4 py-3 backdrop-blur">
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <button onClick={() => go("onlineCart")} className="relative rounded-full border border-[#E8D8B8] bg-white p-2">
          <ShoppingBag className="h-5 w-5" />
          {itemCount > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-[#FFC72C] px-1.5 text-xs font-black">{itemCount}</span>}
        </button>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-5xl p-4 pb-10">
      <AnimatePresence mode="wait">
        <motion.div key={screen} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.18 }}>
          {screen === "home" && <section className="space-y-4">
            <div className="rounded-[2rem] bg-white p-5 shadow-lg ring-1 ring-[#E8D8B8]">
              <Pill active>Smart cafeteria system</Pill>
              <h2 className="mt-5 text-4xl font-black leading-tight"><span className="text-[#FFC72C]">Order online</span> or pay by NFC tray.</h2>
              <p className="mt-4 text-sm leading-6 text-slate-600">Choose one path. Each section opens as a separate screen, so mobile users do not need to scroll through the whole page.</p>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              <button onClick={() => go("onlineShops")} className="rounded-[2rem] border border-[#E8D8B8] bg-white p-5 text-left shadow-sm">
                <MonitorSmartphone className="h-8 w-8 text-[#003E74]" />
                <h3 className="mt-4 text-2xl font-black">Online ordering</h3>
                <p className="mt-2 text-sm text-slate-500">Choose shop → menu → cart → pickup.</p>
                <div className="mt-5 rounded-full bg-[#FFC72C] px-4 py-3 text-center font-black">Start online order</div>
              </button>
              <button onClick={() => go("nfcScan")} className="rounded-[2rem] border border-[#E8D8B8] bg-white p-5 text-left shadow-sm">
                <Nfc className="h-8 w-8 text-[#003E74]" />
                <h3 className="mt-4 text-2xl font-black">NFC smart tray</h3>
                <p className="mt-2 text-sm text-slate-500">Scan food already placed on the tray.</p>
                <div className="mt-5 rounded-full bg-[#FFC72C] px-4 py-3 text-center font-black">Open tray mode</div>
              </button>
            </div>
            <button onClick={() => go("onlineSeats")} className="w-full rounded-[2rem] border border-[#E8D8B8] bg-white p-5 text-left shadow-sm">
              <div className="flex items-center justify-between"><div><h3 className="text-2xl font-black">Seat availability</h3><p className="mt-1 text-sm text-slate-500">Check Area A / B before dining.</p></div><Armchair className="h-8 w-8 text-[#003E74]" /></div>
            </button>
          </section>}
